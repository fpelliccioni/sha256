...
std::array<uint32_t, 8> sha256(const std::vector<uint8_t> &message) {
    ...
    for (size_t i = 0; i < message_length; i += 64) {
        ...
        for (size_t j = 16; j < 64; ++j) {
            uint32_t s0 = (w[j - 15] >>  7 | w[j - 15] << 25) ^ (w[j - 15] >> 18 | w[j - 15] << 14) ^ (w[j - 15] >>  3);            // σ0
            uint32_t s1 = (w[j -  2] >> 17 | w[j -  2] << 15) ^ (w[j -  2] >> 19 | w[j -  2] << 13) ^ (w[j -  2] >> 10);            // σ1
